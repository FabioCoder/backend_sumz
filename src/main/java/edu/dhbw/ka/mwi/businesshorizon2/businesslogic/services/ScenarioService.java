...
public class ScenarioService implements IScenarioService{
        ...
        //why would you need request 500 samples (excact same prediction -> unnecessary load)
	...
	public Long create(ScenarioPostRequestDto scenarioDto, Long appUserId) {
		...
		HashMap<MultiPeriodAccountingFigureNames, HashMap<Integer, List<Double>>> stochasticAccountingFigures = 
                ...
                //contains all of the required variables (including the time series etc.) for company valuation if the user enters future values?
		...
		nonHistoricAccountingFigures.removeIf(x -> x == null || x.getTimeSeries() == null || x.getIsHistoric() == null || x.getIsHistoric().equals(true));
                ...
                //contains all of the required variables for company valuation if the user enters historical values?
		...
		double effectiveTaxRate = accountingService.calculateEffectiveTaxRate(scenarioDto.getBusinessTaxRate(), scenarioDto.getCorporateTaxRate(), scenarioDto.getSolidaryTaxRate());
                ...
                //for each figure/variable needed to calculate the company value:
		...
		for (MultiPeriodAccountingFigureRequestDto figure : nonHistoricAccountingFigures) {
			...
			if(figure.getFigureName() == MultiPeriodAccountingFigureNames.FreeCashFlows) {
                ...
                //if the nonhistoricaccountingfigures do not contain free cash flows, check whether free cash flows are provided for the historic accounting figures -> why not first distinguish whether we have historic or non historic input
                // so we don't need to check this for both cases ?? -> own methods
